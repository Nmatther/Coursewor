...
function findItemById(id) {
  ...
  const searchId= items.find((element) => element.id===id);
...
function calculateTotalInventory() {
  // TODO Use the .reduce() method to return the total number of items in inventory
   return items.reduce((accum, currentItem) => {
      return accum + currentItem.inventory;
   }, 0);
}
...
function calculateAllInventoryPrice() {
  ...
  return items.reduce((accum, currentItem) => {
    const totalPrice = currentItem.price * currentItem.inventory;
...
function getItemPriceByName(name) {
  ...
  const searchName= items.find((element) => element.name===name);


  return searchName.price;
}
...
function filterItemsByCategoryId(categoryId) {
  // TODO: use the .filter() method to filter out all items which don't belong the passed in category
   let filteredItems= items.filter((currentItem)=> currentItem.categoryId ===categoryId);
...
function calculateTotalCartPrice() {
  ...
  return cart.reduce((accum, currentItem) => {
    const totalPrice = currentItem.price * currentItem.inventory;
    return accum + totalPrice;},0);
  }
